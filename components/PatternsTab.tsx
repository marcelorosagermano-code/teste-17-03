...
import { Search, Layers, ArrowRight, Aperture, Zap, Plus } from 'lucide-react';

const CategoryCard: React.FC<{ category: any, categoryCounts: any, onOrderFromPattern: (pattern: any) => void }> = ({ category, categoryCounts, onOrderFromPattern }) => {
  ...
  const images = (category as any).images && (category as any).images.length > 0 
  ...
  useEffect(() => {
    ...
    const idNum = parseInt(category.id.replace(/\D/g, '')) || 0;
    const intervalTime = 10000 + (idNum * 1337 % 10001);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const x = Math.sin(elapsed * 0.5) * 0.15;
      const y = Math.cos(elapsed * 0.7) * 0.15;
  ...
  return (
    <div 
      ...
      className="group relative h-[400px] sm:h-[450px] rounded-[2rem] overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 hover:border-rose-500/30 transition-all duration-700 transform-gpu isolation-isolate hover:scale-[1.02] hover:z-30 hover:shadow-xl dark:hover:shadow-[0_20px_50px_rgba(244,63,94,0.2)] will-change-transform"
    >
      <div 
          ...
          <div className="flex flex-col items-center justify-center mt-6 gap-3">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="px-6 py-3.5 rounded-full bg-white text-black flex items-center gap-2 hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl hover:shadow-white/20 cursor-pointer z-30"
              >
              ...
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  onOrderFromPattern(category);
                }}
                className="p-3.5 rounded-full bg-brand-600 text-white flex items-center justify-center hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl hover:shadow-brand-500/20 cursor-pointer z-30"
                title="Criar pedido com este molde"
              >
                <Plus size={20} />
              </button>
            </div>
...
export const PatternsTab: React.FC<{ onOrderFromPattern: (pattern: any) => void }> = ({ onOrderFromPattern }) => {
  ...
  const categoryCounts: { [key: string]: number } = {
    '01': 6, '02': 11, '03': 11, '04': 3, '05': 6,
    '06': 6, '07': 13, '08': 5, '09': 13, '10': 7,
    '11': 17, '12': 26, '13': 15, '14': 14, '15': 49
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#050505] text-zinc-900 dark:text-white selection:bg-rose-500/30 pb-40 relative overflow-hidden transition-colors duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredCategories.map((category) => (
            <CategoryCard 
              ...
              onOrderFromPattern={onOrderFromPattern}
            />
          ))}
        </div>
